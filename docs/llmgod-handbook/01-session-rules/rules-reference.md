...
| 客户端 CYBER_RISK 拦截 | Let's LLM | Runtime patch 移除拦截指令 |
| Feature Flag 限制 | Let's LLM | GrowthBook override |
| 工具调用前阻断 | Let's LLM + Guard Hook | Patch 移除限制 + Hook 重建可控边界 |

**只有 Rules 没有 Let's LLM**：模型愿意回答但客户端仍会拦截。
**只有 Let's LLM 没有 Rules**：客户端不拦截但模型仍然会拒绝。
**两者配合**：双层限制同时解除。
