...
Let's LLM 解决客户端层。Session Rules 解决模型层。Guard 重建可控边界。
...
1. 安装 [Let's LLM](https://github.com/syntharea/llmgod) — 解除客户端层限制
...
```bash
# macOS / Linux
cd llmgod-handbook/hooks && bash setup.sh

# Windows (PowerShell)
cd llmgod-handbook\hooks; .\setup.ps1
```
...
└── appendix/
    ...
    ├── llmgod-integration.md            # 与 Let's LLM 配合说明
...
用户输入
    ↓
Layer 0: Let's LLM Runtime Patch    ← 解除客户端黑箱限制
    ↓
...
3. **最小配置起效** — Let's LLM + 一份 Rules 文件就能解决 80% 的问题
...
与 Let's LLM 项目独立，不要求安装 Let's LLM 即可使用本文档。
