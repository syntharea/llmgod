...
$LlmgodDir = Join-Path $env:USERPROFILE ".llmgod"
...
Write-Host ""
Write-Host "  Let's LLM Installer" -ForegroundColor White -NoNewline
...
if ($Uninstall) {
    ...
    # Remove explicit llmgod alias
    $llmgodCmd = Join-Path $BinDir "llmgod.cmd"
    if (Test-Path $llmgodCmd) {
        Remove-Item -Force $llmgodCmd
        Write-OK "Removed llmgod alias"
    }
    ...
    foreach ($f in @("cli.js","cli.cjs","cli.original.js","cli.original.cjs","cli.original.js.bak","cli.original.cjs.bak","patch.js","patch.mjs","extract-natives.mjs","post-process.mjs","repatch.mjs",".source-version","node_modules","bun-runtime","vendor")) {
        $p = Join-Path $LlmgodDir $f
        if (Test-Path $p) { Remove-Item -Recurse -Force $p }
    }
    Write-OK "Let's LLM uninstalled"
    Write-Host ""
...
New-Item -ItemType Directory -Force -Path $LlmgodDir | Out-Null
...
# the registry. Every one of those is a stale-source trap: llmgod patches
...
# llmgod was first installed. `claude update` (which is now redirected
...
if (-not $NativeBin) {
    ...
    $NativeBinTmpDir = Join-Path $env:TEMP "llmgod-binary-$([Guid]::NewGuid().ToString('N'))"
...
$extractorPath = Join-Path $LlmgodDir "extract-natives.mjs"
@'
#!/usr/bin/env node
/**
...
$VendorDir = Join-Path $LlmgodDir "vendor"
if (Test-Path $VendorDir) { Remove-Item -Recurse -Force $VendorDir }
New-Item -ItemType Directory -Force -Path $VendorDir | Out-Null

$dstCli = Join-Path $LlmgodDir "cli.original.js"

Write-Dim "Extracting cli.js from $NativeBinLabel ..."
& node $extractorPath $NativeBin $LlmgodDir --cli-js 2>&1 | ForEach-Object { Write-Host "  $_" }
...
$postProc = Join-Path $LlmgodDir "post-process.mjs"
@'
...
if (-not (Test-Path (Join-Path $LlmgodDir "cli.original.cjs"))) {
    Write-Err "Post-process failed"
    exit 1
}

# Stamp source version so wrapper can detect drift on next launch
Set-Content -Path (Join-Path $LlmgodDir ".source-version") -Value $NativeBinLabel -Encoding ASCII
...
'@ | Set-Content (Join-Path $LlmgodDir "repatch.mjs") -Encoding UTF8
...
// llmgod installs (we patch out `claude update`), so the check could only
...
// llmgod: default-on ultracode — xhigh effort + standing dynamic-workflow
// orchestration. Seed ~/.claude/settings.json never-clobber: only when the key
// is absent, so an explicit user value (including false) always wins.
// enableWorkflows keeps the workflow half on even on Pro plans.
try {
...
'@ | Set-Content (Join-Path $LlmgodDir "cli.cjs") -Encoding UTF8
...
const patches = [
  ...
  {
    // Redirect CLI `claude update` to llmgod self-update. Upstream's
  ...
  },
  {
    ...
    // backup llmgod creates at install time). Then the multitool dispatch
...
Set-Content (Join-Path $LlmgodDir "patch.mjs") $patcherCode -Encoding UTF8
Write-OK "Patcher created (patch.mjs)"

# ─── Apply patches ────────────────────────────────────

Write-Dim "Applying patches ..."
node (Join-Path $LlmgodDir "patch.mjs")

# ─── Create default configs ───────────────────────────

$featuresFile = Join-Path $LlmgodDir "features.json"
...
$sanityCli = Join-Path $LlmgodDir "cli.cjs"
...
$cliPathInCmd = "%USERPROFILE%\.llmgod\cli.cjs"
...
$launcherContent = "@echo off`r`nif not exist `"$cliPathInCmd`" (`r`n  echo llmgod: cli.cjs not found. Reinstall: irm https://github.com/syntharea/llmgod/releases/latest/download/install.ps1 ^| iex`r`n  exit /b 127`r`n)`r`nif not exist `"$bunPathInCmd`" (`r`n  echo llmgod: bun not found at $bunPathInCmd. Install: https://bun.sh/install`r`n  exit /b 127`r`n)`r`n`"$bunPathInCmd`" `"$cliPathInCmd`" %*"
...
# Write .cmd launcher for both 'claude' and the explicit 'llmgod' alias.
# Why both:
#  - claude.cmd may be shadowed by a claude.exe higher in PATH
#  - llmgod.cmd has no .exe competitor, so it always works
#  - User can invoke patched explicitly via `llmgod` regardless of which
#    binary 'claude' resolves to
foreach ($cmd in @("claude", "llmgod")) {
    $launcherContent | Set-Content (Join-Path $BinDir "$cmd.cmd") -Encoding Default
}
Write-OK "Commands 'claude' + 'llmgod' → patched"
...
Write-Host ""
Write-Host "  Let's LLM installed!" -ForegroundColor Green
Write-Host ""
...
Write-Dim "  in one step. To leave llmgod and use vanilla update:"
Write-Dim "    bash ~/.llmgod/install.sh --uninstall"
Write-Host ""
Write-Err "  If 'claude' still runs the old version, restart your terminal."
Write-Host ""
Write-Dim "  Config: ~/.llmgod/provider.json"
Write-Dim "  Flags:  ~/.llmgod/features.json"
Write-Host ""
